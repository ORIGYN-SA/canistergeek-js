...
const LogMessagesDataProvider = (props) => {
    ...
    const getInfos = (0, react_1.useCallback)(async (canisterIds) => {
        try {
            ...
            const promises = lodash_1.default.map(canisterIds, (canisterId) => fetchCanisterInfo(canisterId, props.createActorFn, props.identity, props.host));
    ...
    const getLogMessages = (0, react_1.useCallback)(async (parameters) => {
        ...
        const promises = lodash_1.default.map(parameters.canisters, async (canisterContext) => {
            const canisterActor = await props.createActorFn(canisterContext.canisterId, canistergeek_1.idlFactory, {
                agentOptions: {
...
const fetchCanisterInfo = async (canisterId, createActorFn, identity, host) => {
    const canisterActor = await createActorFn(canisterId, canistergeek_1.idlFactory, {
        agentOptions: {
            identity: identity,
            host: host
        }
...
const getCanisterLogRecursive = async (canisterId, canisterActor, request) => {
    ...
    if ((0, typescriptAddons_1.hasOwnProperty)(request, "getMessages")) {
        ...
        console.log("getLatestMessages request", request);
    ...
    while (true) {
        ...
        if (loggerResponse && (0, typescriptAddons_1.hasOwnProperty)(loggerResponse, "messages")) {
            console.log("loggerResponse", loggerResponse);
            const logMessages = loggerResponse.messages;
            messagesResult.data = [
                ...messagesResult.data,
                ...logMessages.data
