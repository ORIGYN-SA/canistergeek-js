...
} from '../dataProvider/ConfigurationProvider';
import _ from 'lodash';
import { LogMessagesSummaryRefreshComponent } from './LogMessagesSummaryRefreshComponent';

const getContentComponent = (
    section: LogMessagesSection
): [React.ReactNode, React.ReactNode] => {
    switch (section) {
        case 'summary':
            return [
                <LogMessagesSummarySectionComponent />,
                <Space direction={'horizontal'}>
                    <LogMessagesSummaryRefreshComponent />
                </Space>,
            ];
        case 'realtime':
...
export const CanistergeekLogMessagesPage = () => {
    ...
    const atLeastOneCanisterInfoLoaded = _.some(
        logMessagesDataContext.infoStatus,
        (value) => value.loaded
    );

    const onClickMenu = useCallback(({ key }) => {
        history.push(
            urlPathContext.pathToLogMessagesSection(key as LogMessagesSection)
        );
    }, []);

    const canisterIds: Array<CanisterId> =
        configurationContext.configuration.canisters.map((v) => v.canisterId);
    ...
    if (
        !_.includes(
            [
                'summary' as LogMessagesSection,
                'realtime' as LogMessagesSection,
                'history' as LogMessagesSection,
            ],
            sectionInURL
        )
    ) {
        return (
            <Redirect to={urlPathContext.pathToLogMessagesSection('summary')} />
        );
    }
    console.log("Log messages context : ", logMessagesDataContext)
    ...
    return (
        <>
            <PageHeader
                title={`${PRODUCT_NAME}: Log Messages`}
                // title={`origyn nft: Log Messages`}
            ...
            <PageContent>
                ...
                <Row>
                    <Col flex={'auto'}>
                        <Menu
                            onClick={onClickMenu}
                            selectedKeys={[sectionInURL]}
                            mode="horizontal"
                        >
                            <Menu.Item key={'summary' as LogMessagesSection}>
                                Summary
                            </Menu.Item>
                            <Menu.Item key={'realtime' as LogMessagesSection}>
                                Realtime
                            </Menu.Item>
                            <Menu.Item key={'history' as LogMessagesSection}>
                                History
                            </Menu.Item>
                        </Menu>
                    </Col>
